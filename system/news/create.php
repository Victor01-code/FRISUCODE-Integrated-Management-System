...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($title) || empty($content)) {
        ...
        $extra_media_1 = $_POST['extra_media_1'] ?? null;
        $extra_media_2 = $_POST['extra_media_2'] ?? null;
        $attachment_url = $_POST['attachment_url'] ?? null;
        $attachment_name = trim($_POST['attachment_name'] ?? '');
        ...
        try {
            $stmt = $pdo->prepare("INSERT INTO news (title, content, media_url, media_type, published_date, status, category, author, extra_media_1, extra_media_2, attachment_url, attachment_name) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
            $stmt->execute([$title, $content, $media_url ?: null, $media_url ? $media_type : 'none', $pub_date, $status, $category ?: 'General', $author ?: null, $extra_media_1 ?: null, $extra_media_2 ?: null, $attachment_url ?: null, $attachment_name ?: null]);
...
<head>
    <meta charset="UTF-8">
    <title>FRISUCODE | Add News Article</title>
...
<div class="dashboard-layout">
    ...
    <div class="main">
        ...
        <div class="page-header">
            <h2><i class="fa-solid fa-pen-nib" style="color:var(--primary);margin-right:8px;"></i><?= __('Add News Article') ?></h2>
        ...
        <div class="form-container fade-in">
            ...
            <form action="create.php" method="POST" enctype="multipart/form-data" id="newsForm">

                <div class="input-group">
                    <label><i class="fa-solid fa-heading"></i> <?= __('Article Title *') ?></label>
                    <input type="text" name="title" required placeholder="<?= __('e.g. School Renovation Completed in Nambala') ?>" value="<?= htmlspecialchars($_POST['title'] ?? '') ?>">
                </div>
                ...
                <div class="form-row">
                    ...
                    <div class="input-group">
                        ...
                        <input type="text" name="author" placeholder="<?= __('e.g. FRISUCODE Team') ?>" value="<?= htmlspecialchars($_POST['author'] ?? '') ?>">
                    </div>
                </div>

                <div class="input-group">
                    <label><i class="fa-solid fa-align-left"></i> <?= __('Article Content *') ?></label>
                    <textarea name="content" rows="10" required placeholder="<?= __('Write the full article content here...') ?>"><?= htmlspecialchars($_POST['content'] ?? '') ?></textarea>
                </div>

                <div class="form-row">
                    <div class="input-group">
                        <label><i class="fa-solid fa-calendar-day"></i> <?= __('Publish Date') ?></label>
                        <input type="date" name="published_date" value="<?= htmlspecialchars($_POST['published_date'] ?? date('Y-m-d')) ?>">
                    </div>
                    <div class="input-group">
                        <label><i class="fa-solid fa-eye"></i> <?= __('Status') ?></label>
                        <select name="status">
                            <option value="published" <?= (($_POST['status'] ?? 'published') === 'published') ? 'selected' : '' ?>><?= __('Published') ?> – <?= __('Visible on website') ?></option>
                            <option value="draft" <?= (($_POST['status'] ?? '') === 'draft') ? 'selected' : '' ?>><?= __('Draft') ?> – <?= __('Hidden from public') ?></option>
                        </select>
                    </div>
                </div>
                ...
                <div class="input-group" style="margin-top:10px;">
                    ...
                    <div class="radio-cards-grid">
                        <label class="radio-card <?= (($_POST['media_type'] ?? 'none') === 'none') ? 'active' : '' ?>">
                            <input type="radio" name="media_type" value="none" <?= (($_POST['media_type'] ?? 'none') === 'none') ? 'checked' : '' ?> style="display:none;" onchange="toggleMediaUrl(this)">
                        ...
                        <label class="radio-card <?= (($_POST['media_type'] ?? '') === 'image') ? 'active' : '' ?>">
                            <input type="radio" name="media_type" value="image" <?= (($_POST['media_type'] ?? '') === 'image') ? 'checked' : '' ?> style="display:none;" onchange="toggleMediaUrl(this)">
                        ...
                        <label class="radio-card <?= (($_POST['media_type'] ?? '') === 'video') ? 'active' : '' ?>">
                            <input type="radio" name="media_type" value="video" <?= (($_POST['media_type'] ?? '') === 'video') ? 'checked' : '' ?> style="display:none;" onchange="toggleMediaUrl(this)">
                ...
                <div class="input-group" id="mediaUrlGroup" style="<?= (($_POST['media_type'] ?? 'none') === 'none') ? 'display:none;' : '' ?>">
                    <label><i class="fa-solid fa-link"></i> <?= __('Media URL') ?></label>
                    <input type="url" name="media_url" id="mediaUrlInput" placeholder="https://example.com/image.jpg" value="<?= htmlspecialchars($_POST['media_url'] ?? '') ?>" style="margin-bottom:10px;">
                ...
                <div style="margin-top:30px;padding-top:20px;border-top:1px solid #f1f5f9;">
                    ...
                    <div class="form-row">
                        <div class="input-group">
                            ...
                            <input type="url" name="extra_media_1" placeholder="URL" value="<?= htmlspecialchars($_POST['extra_media_1'] ?? '') ?>" style="margin-bottom:10px;">
                        ...
                        <div class="input-group">
                            ...
                            <input type="url" name="extra_media_2" placeholder="URL" value="<?= htmlspecialchars($_POST['extra_media_2'] ?? '') ?>" style="margin-bottom:10px;">
                ...
                <div style="margin-top:20px;padding-top:20px;border-top:1px solid #f1f5f9;">
                    ...
                    <div class="form-row">
                        <div class="input-group">
                            ...
                            <input type="url" name="attachment_url" placeholder="URL" value="<?= htmlspecialchars($_POST['attachment_url'] ?? '') ?>" style="margin-bottom:10px;">
                        ...
                        <div class="input-group">
                            ...
                            <input type="text" name="attachment_name" placeholder="<?= __('e.g. Full Financial Report Q1') ?>" value="<?= htmlspecialchars($_POST['attachment_name'] ?? '') ?>">
                        </div>
                    </div>
                </div>
                ...
                <div style="margin-top:40px;padding-top:20px;border-top:1px solid #f1f5f9;">
                    <button type="submit" class="btn-primary-block">
                        <i class="fa-solid fa-paper-plane"></i> <?= __('Publish Article') ?>
                    </button>
                    <p style="text-align:center;font-size:0.85rem;color:#94a3b8;margin-top:20px;">
                        <i class="fa-solid fa-globe"></i> <?= __('Published articles appear immediately on the public website.') ?>
                    </p>
                </div>
